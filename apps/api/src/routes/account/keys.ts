...
function serializeKey(id: string, data: FirebaseFirestore.DocumentData) {
  return {
    ...
    isSessionKey:  data.isSessionKey === true,
...
// GET /api/account/keys/:id/logs — riwayat request terbaru untuk satu key
app.get('/:id/logs', async (c) => {
  ...
  const snap = await ref.collection('logs').orderBy('timestamp', 'desc').limit(50).get()
  ...
  return c.json({ status: true, statusCode: 200, creator: 'Xena', result: logs })
})
